...
NOTE: 일반 Snap Creator 오류에 대한 정보는 다음을 참조하세요.link:../administration/index.html["Snap Creator Framework 4.3.3 관리 가이드"] .
...
** Windows 호스트에서는 IBM Domino 설치 경로를 PATH 환경 변수에 추가해야 합니다. 자세한 내용은 다음을 참조하세요.link:https://docs.netapp.com/us-en/snap-creator-framework/installation/task_configure_windows_specific_settings_adding_path_to_the_environment_variables.html["Snap Creator Framework 설치 가이드 - 환경 변수에 경로 추가"] .
...
기본적으로 하나 이상의 데이터베이스가 일관되지 않거나 손상된 상태이면 백업 작업이 실패합니다. 오류가 발생하더라도 플러그인이 백업 작업을 계속 진행하도록 강제할 수 있습니다. 자세한 내용은 link:task_using_the_gui_to_create_a_configuration_file.html["GUI를 사용하여 구성 파일을 생성하는 작업"] 단계를 참조하십시오.
...
Domino 플러그인은 snap_timestamp_only=N과 함께 사용할 수 없습니다 snap_timestamp_only를 Y로 설정해야 합니다
 a| 
자세한 내용은 link:task_using_the_gui_to_create_a_configuration_file.html["GUI를 사용하여 구성 파일을 생성하는 작업"] 단계를 참조하십시오.
