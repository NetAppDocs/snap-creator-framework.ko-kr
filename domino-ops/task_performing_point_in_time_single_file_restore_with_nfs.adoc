...
NOTE: SnapVault 복사본에서 복원하는 경우 운영 스토리지 또는 2차 스토리지에서 복원하라는 메시지가 표시됩니다. 선택은 link:concept_domino_database_restore_overview.html["concept_domino_database_restore_overview.md#GUID-4D864E27-DE54-43BF-9B9F-EF2F240F65D9"]에 설명된 제한 사항을 처리하는 방식에 따라 달라집니다.
