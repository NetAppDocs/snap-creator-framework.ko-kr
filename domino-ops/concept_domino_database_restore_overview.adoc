...
볼륨 복원과 단일 파일 복원은 일반적으로 서로 다른 대상 디렉터리를 사용합니다. 백업 및 복원 구성 파일의 Data Restore Directory 매개 변수에서 대상 디렉터리를 지정합니다(단계 link:task_using_the_gui_to_create_a_configuration_file.html["GUI를 사용하여 구성 파일을 생성하는 작업"] 참조):
